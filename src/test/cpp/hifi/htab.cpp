...
typedef struct {int n, m; uint64_t n_ins; uint64_t *a; ha_mz1_t *b;} mz1_ch_buf_t;
static inline void mz1_ct_insert_buf(mz1_ch_buf_t *buf, int p, uint64_t y) /** insert a k-mer $y to a linear buffer**/
{
    /**assign k-mer to one of the 4096 bins**/
    /**using low 12 bits for assigning**/
    /**so all elements at b have the same low 12 bits**/
    int pre = y & ((1<<p) - 1);
    mz1_ch_buf_t *b = &buf[pre];
    if (b->n == b->m) {
        b->m = b->m < 8? 8 : b->m + (b->m>>1);
        REALLOC(b->a, b->m);
    }
...
/**buf is the read block, k is the k-mer length, p = 12, len is the read length, seq is the read**/
static void mz1_count_seq_buf(mz1_ch_buf_t *buf, int k, int p, int len, const char *seq) /**insert k-mers in $seq to linear buffer $buf**/
{
    int i, l;
    uint64_t x[4], mask = (1ULL<<k) - 1, shift = k - 1;
    for (i = l = 0, x[0] = x[1] = x[2] = x[3] = 0; i < len; ++i) {
        ...
        if (c < 4) { /** not an "N" base**/
            ...
            x[3] = x[3] >> 1 | (uint64_t)(1 - (c>>1)) << shift;
            if (++l >= k)
                mz1_ct_insert_buf(buf, p, yak_hash_long(x));
        } else l = 0, x[0] = x[1] = x[2] = x[3] = 0; /** if there is an "N", restart**/
    }
}
static void mz1_count_seq_buf_HPC(mz1_ch_buf_t *buf, int k, int p, int len, const char *seq) /**insert k-mers in $seq to linear buffer $buf**/
{
    int i, l, last = -1;
    uint64_t x[4], mask = (1ULL<<k) - 1, shift = k - 1;
    for (i = l = 0, x[0] = x[1] = x[2] = x[3] = 0; i < len; ++i) {
...
int mz1_ha_pt_insert_list(ha_pt_t *h, int n, const ha_mz1_t *a)
{
    int j, mask = (1<<h->pre) - 1, n_ins = 0;
    ha_pt1_t *g;
    if (n == 0) return 0;
...
/** data structure for each step in kt_pipeline()**/
typedef struct {pl_data_t *p;uint64_t n_seq0; int n_seq, m_seq, sum_len, nk, uq, *len; char **seq; ha_mz1_v *mz_buf; ha_mz1_v *mz;mz1_ch_buf_t *buf;st_mt_t *mt;} mz1_st_data_t;
static void mz1_worker_for_insert(void *data, long i, int tid) /** callback for kt_for()**/
{
    mz1_st_data_t *s = (mz1_st_data_t*)data;
    mz1_ch_buf_t *b = &s->buf[i];
    if (s->p->pt){
        if(s->p->flag&HAF_COUNT_REFINE) b->n_ins += ha_pt_cnt_insert_list(s->p->pt, b->n, b->a);
        else b->n_ins += mz1_ha_pt_insert_list(s->p->pt, b->n, b->b);
    }else{
        b->n_ins += ha_ct_insert_list(s->p->ct, s->p->create_new, b->n, b->a);
    }
}
static void mz1_worker_for_mz(void *data, long i, int tid)
{
    mz1_st_data_t *s = (mz1_st_data_t*)data;
    /**get the corresponding minimzer vector of this read**/
    ha_mz1_v *b = &s->mz_buf[tid];
    s->mz_buf[tid].n = 0;
    mz1_ha_sketch(s->seq[i], s->len[i], s->p->opt->w, s->p->opt->k, s->n_seq0 + i, s->p->opt->is_HPC, b, s->p->flt_tab, asm_opt.mz_sample_dist, 0, 0,
                  (s->p->pt&&(s->p->flag&HAF_COUNT_REFINE))?s->p->pt:NULL, s->p->opt->min_rcnt, asm_opt.dp_min_len, asm_opt.dp_e, &(s->mt[tid]), asm_opt.mz_rewin, s->uq);
    s->mz[i].n = s->mz[i].m = b->n;
    MALLOC(s->mz[i].a, b->n);
    memcpy(s->mz[i].a, b->a, b->n * sizeof(ha_mz1_v));
}
static inline void mz1_pt_insert_buf(mz1_ch_buf_t *buf, int p, const ha_mz1_t *y){
    /**assign minimizer to one of 4096 bins by low 12 bits**/
    int pre = y->x & ((1<<p) - 1);
    mz1_ch_buf_t *b = &buf[pre];
    if (b->n == b->m) {
...
static void *mz1_worker_count(void *data, int step, void *in) /** callback for kt_pipeline()**/
{
    pl_data_t *p = (pl_data_t*)data;
    if (step == 0) { /** step 1: read a block of sequences**/
        int ret;
        ...
        if (p->rs_in && (p->flag & HAF_RS_READ)) {
            ...
            while ((ret = kseq_read(p->ks)) >= 0) {
                ...
                if (p->rs_out) {
                    // just realloc all the seq length -song
                    /**for 0-th count, just insert read length to R_INF, instead of read**/
                    if (p->flag & HAF_RS_WRITE_LEN) {
                        assert(p->n_seq == p->rs_out->total_reads);
                        ha_insert_read_len(p->rs_out, l, p->ks->name.l);
        ...
        /**s is the block of reads**/
        mz1_st_data_t *s = (mz1_st_data_t*)in;
        int i, n_pre = 1<<p->opt->pre, m;
        /**allocate the k-mer buffer**/
        CALLOC(s->buf, n_pre);
        m = (int)(s->nk * 1.2 / n_pre) + 1;
        ...
        if (p->opt->w == 1) { /** enumerate all k-mers**/
            ...
            for (i = 0; i < s->n_seq; ++i) {
                if (p->opt->is_HPC)
                    mz1_count_seq_buf_HPC(s->buf, p->opt->k, p->opt->pre, s->len[i], s->seq[i]);
                else
                    mz1_count_seq_buf(s->buf, p->opt->k, p->opt->pre, s->len[i], s->seq[i]);
                if (!p->is_store) free(s->seq[i]);
            }
        } else { /** minimizers only**/
            uint32_t j;
            ...
            CALLOC(s->mz, s->n_seq), CALLOC(s->mz_buf, p->opt->n_thread), CALLOC(s->mt, p->opt->n_thread);
            /**calculate minimzers for each read, each read corresponds to one thread**/
            kt_for(p->opt->n_thread, mz1_worker_for_mz, s, s->n_seq);
            for (i = 0; i < p->opt->n_thread; ++i) free(s->mt[i].a), free(s->mz_buf[i].a);
            free(s->mt), free(s->mz_buf);
            /**insert minimizers**/
            if (p->pt && !(p->flag&HAF_COUNT_REFINE)) {/**insert whole minimizer**/
                for (i = 0; i < s->n_seq; ++i)
                    for (j = 0; j < s->mz[i].n; ++j)
                        mz1_pt_insert_buf(s->buf, p->opt->pre, &s->mz[i].a[j]);
            } else {/**just insert the hash key of minimizer**/
                for (i = 0; i < s->n_seq; ++i)
                    for (j = 0; j < s->mz[i].n; ++j)
                        mz1_ct_insert_buf(s->buf, p->opt->pre, s->mz[i].a[j].x);
            }
            for (i = 0; i < s->n_seq; ++i) {
                p->n_mz += s->mz[i].n;
                free(s->mz[i].a);
        ...
        int i, n = 1<<p->opt->pre;uint64_t n_ins = 0;
        /**for 0-th counting, p->pt = NULL**/
        kt_for(p->opt->n_thread, mz1_worker_for_insert, s, n);
        /**n_ins is number of distinct k-mers**/
        for (i = 0; i < n; ++i) {
            n_ins += s->buf[i].n_ins;
            if (p->pt && !(p->flag&HAF_COUNT_REFINE)) {
                free(s->buf[i].b);
            }else {
                free(s->buf[i].a);
            }
...
ha_ct_t *ha_count(const hifiasm_opt_t *asm_o, int flag, int HPC, int k, int w, ha_pt_t *p0, const void *flt_tab, All_reads *rs, ma_utg_v *us, int keep_adapter, int *low_freq){
	...
	for (i = n_bs = 0; i < (us?1:asm_o->num_reads); ++i){
        //time consume //in first step, flt_tab is null.
...
void *ha_ft_gen(const hifiasm_opt_t *asm_opt, All_reads *rs, int *hom_cov, int is_hp_mode)
{
    ...
    cout << "reads:" << rs->total_reads << endl;
	...
	if(!(ex_flag & HAF_SKIP_READ))
	{
        ...
        cout<< cnt[1] << endl;//7 cnt -> hist data
